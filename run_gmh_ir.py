...
parser.add_argument('--output_path', type=str, default='experiments/bald_ir/results_gmh_ir.pt')
...
print(f'Models loaded.  sigma={SIGMA:.4f}', flush=True)
...
for trial in range(args.n_trials):
    ...
    log_p_z = log_posterior_ir(z, stylegan, reward_model, prompt_ids, prompt_mask)
    n_accept = 0
    kept     = []

    for step in tqdm(range(1, N_STEPS + 1), desc=f'G_MH trial {trial+1}'):
        z_prop      = z + SIGMA * torch.randn_like(z)
        log_p_prop  = log_posterior_ir(z_prop, stylegan, reward_model, prompt_ids, prompt_mask)
