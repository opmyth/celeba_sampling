#!/usr/bin/env bash
#SBATCH --job-name=traj_male
#SBATCH -p Teaching
#SBATCH --account=general-teaching
#SBATCH --gres=gpu:nvidia_rtx_a6000:1
#SBATCH --nodelist=landonia11
#SBATCH --time=04:00:00
#SBATCH --output=logs/traj_male-%j.out
#SBATCH --error=logs/traj_male-%j.err
...
echo "=== [1/3] Step size sweep (male) ==="
python run_trajectory_stepsize.py --attribute male

echo "=== [2/3] Init comparison — same noise ==="
python run_trajectory_init.py --attribute male --noise same

echo "=== [3/3] Init comparison — indep noise ==="
python run_trajectory_init.py --attribute male --noise indep
