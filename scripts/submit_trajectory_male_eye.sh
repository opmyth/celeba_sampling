#!/usr/bin/env bash
#SBATCH --job-name=traj_male_eye
#SBATCH -p Teaching
#SBATCH --account=general-teaching
#SBATCH --gres=gpu:nvidia_rtx_a6000:1
#SBATCH --nodelist=landonia11
#SBATCH --time=04:00:00
#SBATCH --output=logs/traj_male_eye-%j.out
#SBATCH --error=logs/traj_male_eye-%j.err
...
echo "=== [1/3] Step size sweep ==="
python run_trajectory_stepsize_male_eye.py

echo "=== [2/3] Init comparison — same noise ==="
python run_trajectory_init_male_eye.py --noise same

echo "=== [3/3] Init comparison — indep noise ==="
python run_trajectory_init_male_eye.py --noise indep
