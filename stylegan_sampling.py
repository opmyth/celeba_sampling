...
def run_trials(model, clf, male_clf, dt, sigma, n_chains, n_steps, device, n_trials=10, seed=321):
    ...
    t = time.time()
    ULA_samples = run_sampler_minibatched(latent_ULA_celeba, model, clf, n_chains * n_trials, n_steps, dt, batch_size=64, device=device)
    ...
    t = time.time()
    MALA_samples = run_sampler_minibatched(latent_MALA_celeba, model, clf, n_chains * n_trials, n_steps, dt, batch_size=64, device=device)
    ...
    t = time.time()
    gaussianMH_samples = run_sampler_minibatched(latent_Gaussian_MH_celeba, model, clf, n_chains * n_trials, n_steps, dt, batch_size=64, device=device)
