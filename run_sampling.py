...
def run_trials(model, clf, male_clf, dt, sigma, n_chains, n_steps, device, n_trials=10, seed=321):
    ...
    RS_samples = rejection_sampling(model, clf, n_chains*2, device=device)
    ...
    ULA_samples = latent_ULA_celeba(model, clf, n_chains, n_steps, dt, device=device)[-1]
    ...
    MALA_samples = latent_MALA_celeba(model, clf, n_chains, n_steps, dt, device=device)[-1]
    ...
    gaussianMH_samples = latent_Gaussian_MH_celeba(model, clf, n_chains, n_steps, sigma, device=device)[-1]
...
vae_w2_values, vae_w2_baseline, vae_samples, vae_avg_log_reward, vae_diversity, vae_male_fraction, vae_trial_to_show = run_trials(
    vae, smile_clf, male_clf, dt=args.dt, sigma=args.sigma, n_chains=args.n_chains, n_steps=args.n_steps, n_trials=args.n_trails, device=device
)

dcgan_w2_values, dcgan_w2_baseline, dcgan_samples, dcgan_avg_log_reward, dcgan_diversity, dcgan_male_fraction, dcgan_trial_to_show = run_trials(
    dcgan, smile_clf, male_clf, dt=args.dt, sigma=args.sigma, n_chains=args.n_chains, n_steps=args.n_steps, n_trials=args.n_trails, device=device
)
...
torch.save({
    'vae': {
        ...
        'trial_to_show': vae_trial_to_show
    },
    'dcgan': {
        ...
        'trial_to_show': dcgan_trial_to_show
    }
}, args.output_path)
