...
BASE_DIR       = os.path.join('experiments', 'male_eye', 'trajectory')


def _decode(z_batch, stylegan, clf_male, clf_eye):
    device = next(stylegan.parameters()).device
    with torch.no_grad():
        imgs       = stylegan.G(z_batch.to(device), None)
        p_male     = torch.sigmoid(clf_male(imgs)).squeeze(-1).cpu().tolist()
        p_eye      = torch.sigmoid(clf_eye(imgs)).squeeze(-1).cpu().tolist()
    imgs_np = ((imgs.clamp(-1, 1) + 1) / 2).cpu().numpy()
    return imgs_np.transpose(0, 2, 3, 1), p_male, p_eye
...
def plot_stepsize_grid(stylegan, clf_male, clf_eye, snap_file='stepsize_snapshots.pt'):
    ...
    fig, axes = _make_grid(len(row_labels), len(col_labels), row_labels, col_labels,
                           'Male+Eyeglasses — MALA trajectory by step size')

    for r, dt in enumerate(step_sizes):
        for c, step in enumerate(SNAPSHOT_STEPS):
            z_snap            = snapshots[dt][step]
            imgs, pm, pe      = _decode(z_snap[:1], stylegan, clf_male, clf_eye)
            axes[r, c].imshow(imgs[0])
            axes[r, c].axis('off')
            _annotate(axes[r, c], pm[0], pe[0])
...
def plot_init_grid(stylegan, clf_male, clf_eye, noise='same'):
    ...
    col_labels = [f'Step {s}' for s in SNAPSHOT_STEPS]
    title      = f'Male+Eyeglasses — MALA trajectory by init ({noise} noise)'
    ...
    for chain_idx in range(n_chains):
        ...
        for r, init_type in enumerate(INIT_TYPES):
            for c, step in enumerate(SNAPSHOT_STEPS):
                ...
                imgs, pm, pe = _decode(z_snap[chain_idx:chain_idx+1], stylegan, clf_male, clf_eye)
                axes[r, c].imshow(imgs[0])
                axes[r, c].axis('off')
                _annotate(axes[r, c], pm[0], pe[0])
...
if __name__ == '__main__':
    ...
    if args.plot in ('stepsize', 'all'):
        plot_stepsize_grid(stylegan, clf_male, clf_eye)

    if args.plot in ('init', 'all'):
        noises = ['same', 'indep'] if args.noise == 'both' else [args.noise]
        for n in noises:
            plot_init_grid(stylegan, clf_male, clf_eye, noise=n)
